...
use workflow_verifier::internal::conformance::verifier::{
...
fn injection_trace_starts_at_the_untrusted_resource_and_ends_at_the_command() {
    ...
    let source_id = graph
        ...
        .expect("source node")
        .id;
    let command_id = graph
        .nodes
        .iter()
        .find(|node| node.name == command.name)
        .expect("command node")
        .id;
    ...
    assert_eq!(
        diagnostic
            .trace
            .iter()
            .map(|hop| (hop.label.as_str(), hop.node_id))
            .collect::<Vec<_>>(),
        vec![
            ("untrusted source", source_id),
            ("command sink", command_id),
        ]
    );
}
...
fn injection_correlates_environment_taint_with_exact_quote_boundaries() {
    fn verify_binding(command_source: &str) -> bool {
        ...
        graph.add_node(command.clone());
        graph.add_edge(Edge::simple(EdgeKind::Data, source.id, binding.id));
...
fn authorization_path_graph() -> Graph {
    let mut graph = Graph::empty(Provider::Gitlab, ".gitlab-ci.yml");
    let workflow = Node::simple(
        Provider::Gitlab,
        NodeKind::Workflow,
...
fn authorization_diagnostic_reports_the_ungated_control_path() {
    let graph = authorization_path_graph();
    let id = |name: &str| {
        graph
            .nodes
            .iter()
    ...
    assert_eq!(
        diagnostic
            ...
            .collect::<Vec<_>>(),
        vec![
            ("authorization bypass", id("pipeline")),
            ("authorization bypass", id("release")),
            ("authorization bypass", id("rule:release")),
            ("authorization bypass", id("script:1")),
            ("authorization bypass", id("git push origin HEAD")),
        ]
    );
}
...
fn trusted_manual_gate_dominates_a_privileged_effect() {
    ...
    graph.add_node(command.clone());
    graph.add_entrypoint(manual.id);
...
fn environment_protection_keeps_authorization_unknown_without_a_false_finding() {
    ...
    graph.add_entrypoint(environment.id);
...
fn graph_cycles_and_unknowns_never_become_proved() {
    ...
    graph.add_edge(Edge::simple(EdgeKind::Control, first.id, second.id));
...
fn whole_program_composition_links_cross_file_resource_writes_to_reads() {
    ...
    producer.add_edge(Edge::simple(EdgeKind::Write, producer_job.id, written.id));
    ...
    consumer.add_edge(Edge::simple(EdgeKind::Read, read.id, consumer_job.id));

    let program = compose_program(&[producer, consumer]);
    let written_id = program
        .nodes
        .iter()
        .find(|node| {
            node.name == written.name
                && program.source_path_for(node.span.source) == Some("producer.yml")
        })
        .expect("written resource")
        .id;
    let read_id = program
        .nodes
        .iter()
        .find(|node| {
            node.name == read.name
                && program.source_path_for(node.span.source) == Some("consumer.yml")
        })
        .expect("read resource")
        .id;
    assert!(program.edges.iter().any(|edge| {
        edge.kind == EdgeKind::Persist
            && edge.from == written_id
            && edge.to == read_id
