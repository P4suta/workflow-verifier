...
use workflow_verifier::internal::conformance::foundation::{
    Budget, JsonValue, Span, content_digest, valid_content_digest,
};
use workflow_verifier::internal::conformance::product::{
    Config, ConfigParseOptions, ConfigTrust, DependencySummary, FixProposal, LockEntry, Lockfile,
    PolicyExpectation, PolicyPredicate, PolicyRule, PolicyRuleKind, PolicySelector, Severity,
    Suppression, evaluate_policy, evaluate_policy_fixture, link_local,
};
use workflow_verifier::internal::conformance::syntax::YamlDocument;
use workflow_verifier::internal::conformance::verifier::{Confidence, Diagnostic, Persona};
...
fn config_trust_persona_and_suppression_matching_are_exact() {
    ...
    let program = Graph::empty(Provider::Github, ".github/workflows/ci.yml");
    ...
    for entry in [
        ...
        assert!(config.suppressed(&diagnostic, &program));
    }
    for entry in [
...
fn local_linking_is_recursive_content_addressed_and_cannot_escape() {
    ...
    let root = workflow_verifier::internal::conformance::frontend::compile_auto(
    ...
    let escaping_source =
        "on: push\njobs:\n  build:\n    steps:\n      - uses: ../../outside/action\n";
    let escaping = workflow_verifier::internal::conformance::frontend::compile_auto(
        ".github/workflows/ci.yml",
        escaping_source,
        Budget::default(),
    )
