use workflow_verifier::internal::conformance::foundation::{
    PathError, Position, PublicPath, SourceId, Span, Utf16Position, byte_to_utf16, utf16_to_byte,
};
...
#[test]
fn span_contains_closed_boundaries_and_merge_respects_source_identity() {
    ...
    let earlier = Span::new(SourceId(7), position(0, 1, 1), position(prefix_bytes, 1, 1));
    let merged = span.merge(&earlier);
    assert_eq!(merged.source, SourceId(7));
    assert_eq!(merged.start, earlier.start);
    assert_eq!(merged.stop, span.stop);

    let other_source = Span::new(SourceId(8), Position::default(), Position::default());
    assert_eq!(span.merge(&other_source), span);
    assert_eq!(span.to_string(), "source 7:1:1");
}
