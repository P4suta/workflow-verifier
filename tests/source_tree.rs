...
#[test]
fn malformed_trusted_exclusion_errors_do_not_disclose_the_argument() {
    let repository = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let sensitive_argument = "../private-policy-name".to_owned();
    let error =
        source_snapshot_with_exclusions(&repository, std::slice::from_ref(&sensitive_argument))
            .expect_err("parent traversal must be rejected");

    assert_eq!(
        error,
        "trusted source exclusion at index 0 is not a portable relative prefix"
    );
    assert!(!error.contains(&sensitive_argument));
}
