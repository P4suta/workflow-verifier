...
";
    ...
    assert!(before_script.attributes.get("command").is_some_and(
        workflow_verifier::internal::conformance::domain::AbstractValue::is_untrusted
    ));
    ...
    assert!(source_node.attributes.get("value").is_some_and(
        workflow_verifier::internal::conformance::domain::AbstractValue::is_untrusted
    ));
    assert!(compilation.graph.edges.iter().any(|edge| {
...
"#;
    ...
    assert_eq!(graph.entrypoints, vec![gate.id]);
    assert_eq!(
        gate.unknown,
        Some(UnknownReason::PhaseUnavailable(
...
fn github_permissions_spans_and_default_shell_match_the_reference_contract() {
    ...
    assert!(
        graph
            ...
            .iter()
            .enumerate()
            .all(|(index, node)| node.id.0 == u32::try_from(index).expect("fixture fits u32"))
    );
}
...
#[expect(
    ...
    reason = "one end-to-end contract covers the complete Azure semantic surface"
)]
