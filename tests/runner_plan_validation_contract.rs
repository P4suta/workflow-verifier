...
#[path = "support/valid_request.rs"]
mod valid_request_support;

use std::collections::BTreeMap;

use digest_support::digest;
use valid_request_support::valid_request;
...
fn rejected(
    mutator: impl FnOnce(&mut workflow_verifier::internal::conformance::sandbox::RunnerPlanRequest),
) {
...
fn resolved_native_runtime_identities_do_not_create_false_incompleteness() {
    ...
    macos.scenario = scenario_support::scenario(
        workflow_verifier::internal::conformance::sandbox::RunnerPlatform::MacosArm64,
    );
