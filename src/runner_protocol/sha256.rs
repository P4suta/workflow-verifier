...
// safer than cosmetically renaming or regrouping the reference algorithm.
#![expect(
    clippy::many_single_char_names,
    reason = "FIPS 180-4 names the eight SHA-256 working variables a through h"
)]
#![expect(
    clippy::unreadable_literal,
    reason = "FIPS 180-4 publishes the round constants as fixed-width hexadecimal words"
)]

use crate::internal::runner_protocol::SHA256_HEX_DIGITS;
...
fn compress(state: &mut [u32; SHA256_STATE_WORDS], block: &[u8; SHA256_BLOCK_BYTES]) {
    ...
    let (words, remainder) = block.as_chunks::<SHA256_WORD_BYTES>();
    debug_assert!(remainder.is_empty());
    for (index, word) in words.iter().take(SHA256_INITIAL_SCHEDULE_WORDS).enumerate() {
