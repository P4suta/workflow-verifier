...
use crate::foundation::{Budget, DependencyClass, JsonValue, Position, Span, classify_reference};
use crate::syntax::{MappingEntry, YamlDocument, YamlNode, YamlProblem};
...
fn dependency(
    ...
    Dependency::unresolved(provider, kind, reference, DependencyLocator::Direct, *span)
}
...
fn add_control(graph: &mut Graph, from: &Node, to: &Node) {
    graph.add_edge(Edge::simple(EdgeKind::Control, from.id, to.id));
}

fn add_call(graph: &mut Graph, from: &Node, to: &Node) {
    add_control(graph, from, to);
    graph.add_edge(Edge::simple(EdgeKind::Call, from.id, to.id));
}
...
fn add_github_gate(
    ...
    let gate = Node::new(
        ...
        phase,
        *condition.span(),
        Node::semantics(predicate, attributes, [], [], unknown),
    );
...
fn github_environment(body: &YamlNode) -> Option<(String, Span)> {
    ...
    Some((name.to_owned(), *environment.span()))
}
...
fn add_github_job_resources(graph: &mut Graph, job: &Node, body: &YamlNode) {
    ...
    if let Some(outputs) = body.field("outputs") {
        for entry in mapping(outputs) {
            ...
            graph.add_edge(Edge::simple(EdgeKind::Write, job.id, resource.id));
...
#[expect(
    ...
    reason = "the GitHub workflow schema lowering is reviewed as one exhaustive mapping"
)]
fn lower_github(
    path: &str,
    root: &YamlNode,
    ...
    if let Some(on) = root.field("on") {
        let triggers: Vec<(&str, Span)> = if let Some(value) = on.scalar() {
            ...
            mapping(on)
                .iter()
                .map(|entry| (entry.key.as_str(), entry.span))
                .collect()
        };
    ...
    for (name, body, job) in &jobs {
        ...
        if let Some(reference) = body.field("uses").and_then(YamlNode::scalar) {
            ...
            let call = Node::new(
                ...
                Phase::Plan,
                body.field("uses")
                    .map_or_else(|| *body.span(), |node| *node.span()),
                Node::semantics(
                    Condition::True,
                    BTreeMap::new(),
                    capabilities,
...
mod tests {
    ...
    #[expect(
        clippy::too_many_lines,
        reason = "one exhaustive table covers the expression lexer semantic cross-product"
    )]
...
#[expect(
    clippy::too_many_lines,
    reason = "the GitHub action schema lowering is reviewed as one exhaustive mapping"
)]
fn lower_github_action(
    ...
    if let Some(inputs) = root.field("inputs") {
        for entry in mapping(inputs) {
            ...
            graph.add_node(parameter.clone());
            graph.add_edge(Edge::simple(EdgeKind::Data, parameter.id, workflow.id));
        }
    }
    if let Some(outputs) = root.field("outputs") {
        for entry in mapping(outputs) {
            ...
            graph.add_node(resource.clone());
            graph.add_edge(Edge::simple(EdgeKind::Write, workflow.id, resource.id));
        }
    }
...
#[expect(
    clippy::too_many_lines,
    reason = "GitHub step tagged unions stay adjacent for complete schema review"
)]
fn add_github_steps(
    graph: &mut Graph,
    dependencies: &[Dependency],
...
#[expect(
    clippy::too_many_lines,
    reason = "the GitLab pipeline schema lowering is reviewed as one exhaustive mapping"
)]
fn lower_gitlab(path: &str, root: &YamlNode, dependencies: &[Dependency]) -> Graph {
    ...
    let stage_span = *root.field("stages").unwrap_or(root).span();
...
fn add_gitlab_job_resources(
    ...
    if let Some(environment) = gitlab_effective_field(templates, "environment", body) {
        ...
        graph.add_node(resource.clone());
        graph.add_edge(Edge::simple(EdgeKind::Grant, resource.id, job.id));
    }
    if let Some(cache) = gitlab_effective_field(templates, "cache", body) {
        ...
        graph.add_node(resource.clone());
        graph.add_edge(Edge::simple(EdgeKind::Write, job.id, resource.id));
        graph.add_edge(Edge::simple(EdgeKind::Read, resource.id, job.id));
    }
...
fn add_gitlab_rule_gate(
    ...
    let gate = Node::new(
        ...
        job.phase,
        *expression.span(),
        Node::semantics(condition, attributes, [], [], phase_unknown),
    );
    ...
    let entries: Vec<(String, &YamlNode, Span)> = if mapping(variables).is_empty() {
        sequence(variables)
            ...
            .filter_map(|item| {
                ...
                Some((name, value, *item.span()))
            })
            .collect()
    } else {
        mapping(variables)
            .iter()
            .map(|entry| (entry.key.clone(), &entry.value, entry.span))
            .collect()
    };
...
fn add_azure_repository_resources(
    ...
    for repository in azure_repository_specs(root) {
        ...
        graph.add_node(resource.clone());
        graph.add_edge(Edge::simple(EdgeKind::Read, resource.id, workflow.id));
        graph.add_node(call.clone());
...
#[expect(
    clippy::too_many_lines,
    reason = "Azure step tagged unions stay adjacent for complete schema review"
)]
...
#[expect(
    clippy::too_many_lines,
    reason = "the Azure pipeline schema lowering is reviewed as one exhaustive mapping"
)]
fn lower_azure(
    path: &str,
    root: &YamlNode,
...
fn add_circleci_step(
    ...
    if let Some(builtin) = step_body.scalar() {
        ...
        graph.add_node(call.clone());
        graph.add_edge(Edge::simple(edge_kind, step.id, call.id));
        add_control(graph, step, &call);
        if let Some(orb) = orb_target {
            graph.add_edge(Edge::simple(EdgeKind::Call, call.id, orb.target.id));
        }
        return;
    }
    if let Some(run) = step_body.field("run") {
...
fn link_circleci_invocations(
        ...
        .any(|invocation| visit_cycle(&invocation.alias, invocations, &mut visiting, &mut visited))
    {
        problems.push(FrontendProblem {
            ...
            span: invocations
                .first()
                .map_or_else(Span::default, |invocation| invocation.span),
        });
    }
    problems
}
...
#[expect(
    clippy::too_many_lines,
    reason = "the CircleCI schema lowering is reviewed as one exhaustive mapping"
)]
fn lower_circleci(
    path: &str,
    root: &YamlNode,
    ...
    graph.add_entrypoint(config.id);
