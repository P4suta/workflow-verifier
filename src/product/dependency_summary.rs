use crate::domain::{Capability, ObservableEffect, Provider};
use crate::foundation::{Budget, JsonValue};
use crate::frontend::{Dependency, DependencyKind, DependencyStatus, compile};
use crate::verifier::inferred_effects;
...
fn github_runtime_reasons(document: &crate::syntax::YamlDocument) -> Vec<String> {
    let runtime = document
        .root()
        .and_then(|root| root.field("runs"))
        .and_then(|runs| runs.field("using"))
        .and_then(crate::syntax::YamlNode::scalar);
    match runtime {
