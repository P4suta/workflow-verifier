...
use crate::foundation::{JsonValue, valid_content_digest};
use crate::product::DependencySummary;
...
impl Lockfile {
    ...
    pub fn new(entries: impl IntoIterator<Item = LockEntry>) -> Result<Self, String> {
        ...
        let unsigned = Self::unsigned_json(&unique);
        Ok(Self {
            schema: "lock-v2".to_owned(),
            entries: unique,
            integrity: unsigned.canonical_digest(),
        })
    }

    fn unsigned_json(entries: &[LockEntry]) -> JsonValue {
    ...
    pub fn verify_integrity(&self) -> bool {
        self.schema == "lock-v2"
            && self.integrity == Self::unsigned_json(&self.entries).canonical_digest()
    }
    ...
    /// Parse and authenticate strict lock-v2 product JSON.
    ///
    /// # Errors
