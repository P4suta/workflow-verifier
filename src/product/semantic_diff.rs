use crate::domain::{Capability, EdgeKind, Graph, NodeId, ObservableEffect};
use crate::foundation::{DependencyClass, Digest, JsonValue, classify_reference};
...
fn graph_digest(graph: &Graph) -> String {
    Digest::of_bytes(crate::product::graph_to_canonical_json(
        crate::product::GraphKind::All,
        graph,
    ))
    .to_string()
}
...
fn mutable_calls(graph: &Graph) -> BTreeSet<String> {
    graph
        ...
        .iter()
        .filter(|node| {
            node.kind == crate::domain::NodeKind::Call
...
fn attack_paths(graph: &Graph) -> BTreeSet<PathChange> {
    let sources: Vec<_> = graph
        ...
        .filter(|node| {
            node.attributes
                .values()
                .any(crate::domain::AbstractValue::is_untrusted)
        })
        .collect();
...
fn shortest_path(graph: &Graph, start: NodeId, goal: NodeId) -> Option<Vec<String>> {
    let kinds = [
        EdgeKind::Data,
        EdgeKind::Read,
        EdgeKind::Write,
    ...
    let mut previous: BTreeMap<NodeId, Option<NodeId>> = BTreeMap::from([(start, None)]);
    while let Some(current) = queue.pop_front() {
        if current == goal {
            ...
            path.reverse();
            return Some(path.into_iter().map(|id| id.to_string()).collect());
        }
        ...
        for successor in successors {
            if let std::collections::btree_map::Entry::Vacant(e) = previous.entry(successor) {
...
mod tests {
    ...
    fn path_graph(with_path: bool) -> (Graph, Node, Node) {
        ...
        if with_path {
            ...
            graph.add_edge(Edge::simple(EdgeKind::Control, middle.id, sink.id));
        } else {
        ...
        let source = graph
            ...
            .expect("source survives finalization")
            .clone();
        let sink = graph
            .nodes
            .iter()
