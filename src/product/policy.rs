use crate::domain::{
    AbstractValue, Capability, EdgeKind, Graph, Node, NodeId, NodeKind, ObservableEffect, Provider,
    Trust as ValueTrust,
};
use crate::foundation::{
    DependencyClass, JsonValue, Span, classify_reference, normalize_slashes, valid_content_digest,
};
use crate::frontend::Mutability;
use crate::verifier::{Confidence, Diagnostic, Severity, TraceHop};
...
pub fn evaluate_policy(rules: &[PolicyRule], graph: &Graph) -> Vec<Diagnostic> {
    ...
    nodes.sort_by_key(|left| left.id);
    ...
    for rule in rules {
        ...
        match rule.kind {
            ...
            PolicyRuleKind::Require if matches.is_empty() => diagnostics.push(Diagnostic::new(
                ...
                rule.message.clone(),
                Span::default(),
                Diagnostic::details(Vec::new(), [], [], None),
            )),
        ...
        .filter_map(|sink| {
            ...
            candidates.sort_by(|left, right| {
                left.len().cmp(&right.len()).then_with(|| {
                    left.iter()
                        .map(|node| node.id)
                        .cmp(right.iter().map(|node| node.id))
                })
            });
...
fn predicate_matches(index: &GraphIndex<'_>, node: &Node, predicate: &PolicyPredicate) -> bool {
    match predicate {
        ...
        PolicyPredicate::PathPrefix(prefix) => index
            .graph
            .source_path_for(node.span.source)
            .is_some_and(|path| normalize_slashes(path).starts_with(prefix)),
...
impl<'a> GraphIndex<'a> {
    ...
    fn shortest_path(&self, from: NodeId, to: NodeId) -> Option<Vec<&'a Node>> {
    ...
    fn dominates(&self, dominator: NodeId, node: NodeId) -> bool {
        ...
        let entrypoints: Vec<NodeId> = if self.graph.entrypoints.is_empty() {
            self.nodes
                .keys()
...
mod tests {
    use super::*;
    use crate::domain::{Condition, Edge, Phase, Secrecy, UnknownReason};
    ...
    #[expect(
        ...
        reason = "one exhaustive table covers every policy selector and primitive name"
    )]
    ...
    #[expect(
        clippy::too_many_lines,
        reason = "one exhaustive table covers every policy rule kind and graph outcome"
    )]
    fn graph_index_and_policy_rules_preserve_paths_dominance_and_rule_kinds() {
        ...
        graph.add_entrypoint(source.id);
        ...
        for (from, to) in [
            ...
            graph.add_edge(Edge::simple(EdgeKind::Control, *from, *to));
        }
        let index = GraphIndex::new(&graph);
        assert_eq!(
            index
        ...
        graph.add_edge(Edge::simple(EdgeKind::Control, source.id, sink.id));
