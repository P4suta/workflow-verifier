...
use crate::internal::runner_protocol::{quote_json, sha256_hex};
...
fn normalize_trusted_exclusions(values: &[String]) -> Result<Vec<String>, String> {
    ...
    for (index, value) in values.iter().enumerate() {
        ...
        {
            return Err(format!(
                "trusted source exclusion at index {index} is not a portable relative prefix"
            ));
        }
...
fn read_regular(path: &Path, before: &fs::Metadata) -> Result<Vec<u8>, String> {
    ...
    #[cfg(windows)]
    let same_identity = file_identity(path, &opened)? == file_identity(path, before)?;
    #[cfg(not(windows))]
    let same_identity = file_identity(path, &opened) == file_identity(path, before);
    if !opened.is_file() || !same_identity {
        return Err(format!(
    ...
    #[cfg(windows)]
    let same_identity = file_identity(path, &after)? == file_identity(path, &opened)?;
    #[cfg(not(windows))]
    let same_identity = file_identity(path, &after) == file_identity(path, &opened);
...
fn source_relative(root: &Path, path: &Path) -> Result<String, String> {
...
fn visit_files(
    ...
    if metadata.is_dir() {
        ...
        for entry in entries {
            ...
            let relative = source_relative(root, &path)?;
        ...
        #[cfg(windows)]
        let same_identity = file_identity(current, &after)? == file_identity(current, &metadata)?;
        #[cfg(not(windows))]
        let same_identity = file_identity(current, &after) == file_identity(current, &metadata);
        if !after.is_dir() || !same_identity {
            return Err(format!(
        ...
        let relative = source_relative(root, current)?;
...
#[cfg(not(unix))]
fn set_executable(_path: &Path, _value: bool) {}
