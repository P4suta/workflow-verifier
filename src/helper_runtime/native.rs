...
use crate::internal::helper_runtime::{
    Change, ChangeKind, PrivateSourceTree, ProcessObservation, ScratchTree,
};
...
fn run_native_steps<N: NativeSandbox>(
    plan: &ValidatedPlan,
    source_root: &Path,
    scratch_root: &Path,
...
) -> Result<NativeObservations, LaunchError> {
    let mut observations = NativeObservations {
    ...
    for step in &plan.steps {
        ...
        let working_directory = scratch_root.join(relative);
        ...
        observations
            .redacted_log
            .extend_from_slice(&redact_bytes(&observation.output, secret_values));
        ...
        observations.outcome = observation_outcome(step, &observation);
...
fn finalize_native_run(
    ...
    evidence.append(EvidenceBody::LogRecorded {
        digest: format!(
            "sha256:{}",
            crate::internal::runner_protocol::sha256_hex(&observations.redacted_log)
        ),
    });
    ...
    N: NativeSandbox,
{
    ...
    let observations = run_native_steps(
        plan,
        source_view.path(),
        scratch.path(),
        &secret_values,
    ...
    finalize_native_run(&scratch, evidence, observations)
}
