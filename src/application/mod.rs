...
use crate::domain::{Graph, NodeKind, Provider};
use crate::engine::{
    AnalysisInput, AnalysisOutcome, AnalysisSource, Analyzer, CancellationToken, ConfigSnapshot,
    LoadOptions, LockSnapshot, WorkspaceEntry, WorkspaceFileSystem, WorkspaceLoader,
};
use crate::foundation::{Budget, Digest, JsonValue, content_digest, normalize_slashes};
use crate::frontend::{compile_auto, detect, entrypoint};
use crate::internal::helper_runtime::{
...
use crate::product::{
    ...
    graph_to_canonical_json, graph_to_dot, link_local, resolve_dependencies, semantic_diff,
};
use crate::sandbox::{
...
struct CliWorkspaceFileSystem;

impl WorkspaceFileSystem for CliWorkspaceFileSystem {
...
struct LoadedWorkspace {
    ...
    analysis_sources: Arc<[AnalysisSource]>,
    authenticated: Option<RuntimeSourceSnapshot>,
...
impl AnalysisContext {
    fn analyzer(&self) -> &Analyzer {
        self.analyzer_with(analyzer)
    }

    fn analyzer_with(&self, initialize: impl FnOnce() -> Analyzer) -> &Analyzer {
        self.analyzer.get_or_init(initialize)
    }
}
...
fn dispatch(
    ...
    match command {
        ...
        "__semantic-conformance" => command_semantic_conformance(cwd, rest, context),
...
fn command_semantic_conformance(
    cwd: &Path,
    arguments: &[String],
    context: &AnalysisContext,
) -> Result<CommandOutput, CliError> {
    ...
    let outcome = analyze_target(&target, &common, context)?.result;
    Ok(CommandOutput::success(
        outcome.semantic_conformance().to_canonical_json(),
    ))
}
...
fn parse_sandbox_plan_options(
    cwd: &Path,
    arguments: &[String],
) -> Result<(CommonOptions, Vec<String>, SandboxPlanOptions), CliError> {
...
fn build_sandbox_plan(
    ...
    let (common, positional, options) = parse_sandbox_plan_options(cwd, arguments)?;
    let target = one_target(cwd, &positional)?;
    let analyzed = analyze_sandbox_target(&target, &common, context)?;
    ...
    let scenario = if let Some(path) = options.scenario.as_ref() {
        ...
        scenario_from_options(
            std::slice::from_ref(analyzed.result.program()),
            &backend,
            &options,
        )?
    };
    ...
    let plan = RunnerPlan::build(RunnerPlanRequest {
        ...
        source_digest: analyzed
            ...
            .authenticated
            .as_ref()
            .expect("sandbox analysis always captures a source snapshot")
            .manifest
            .digest
            .clone(),
        lock_digest: analyzed.result.provenance().lock_digest.to_string(),
        controls: required_sandbox_controls(&backend, options.allow_network),
        network_destinations: options.destinations,
        dependencies: sandbox_dependencies(std::slice::from_ref(analyzed.result.program())),
        steps: planned.steps,
        incomplete_reasons,
...
fn scenario_from_options(
    ...
    let mut scenario = Scenario::new(graph.provider(), graph.source_path(), job, event, runner)
        .map_err(CliError::invalid)?;
        ...
        .filter(|node| node.kind == NodeKind::Call && !node.name.starts_with("builtin:"))
    {
        let digest = node
            ...
            .and_then(|values| match values {
                [value] if crate::foundation::valid_content_digest(value) => Some(value.clone()),
                _ => None,
            });
        dependencies
...
fn sandbox_audit(
    ...
    let graphs = if let Some(target) = positional.get(2) {
        let analyzed = analyze_sandbox_target(&absolute(cwd, Path::new(target)), &common, context)?;
        if analyzed
            .loaded
            .authenticated
            .as_ref()
        ...
        Some(analyzed.result.program().clone())
    } else {
        None
    };
    let audit = match graphs.as_ref() {
        Some(graph) => {
            SandboxAudit::evaluate_with_graphs(&plan, &evidence, std::slice::from_ref(graph))
        }
...
fn command_policy(cwd: &Path, arguments: &[String]) -> Result<CommandOutput, CliError> {
    ...
    let passed = results
        .iter()
        .all(crate::product::PolicyFixtureResult::passed);
    let json = JsonValue::Object(BTreeMap::from([
        (
            ...
            JsonValue::Array(
                results
                    .iter()
                    .map(crate::product::PolicyFixtureResult::to_json)
                    .collect(),
            ),
        ),
...
fn evaluate_policy_sidecar(
    ...
    sidecar: &Path,
    config: &Config,
) -> Result<crate::product::PolicyFixtureResult, String> {
    ...
    let diagnostics: Vec<_> = evaluate_policy(&config.rules, &compilation.graph)
        .into_iter()
        .filter(|diagnostic| !config.suppressed(diagnostic, &compilation.graph))
        .collect();
...
fn command_check(
    ...
    let rendered = match format {
        "json" => analyzed.result.check_report().to_canonical_json(),
        "sarif" => analyzed.result.sarif().to_canonical_json(),
...
fn command_graph(
    ...
    Ok(CommandOutput::success(if format == "dot" {
        graph_to_dot(kind, result.program())
    } else {
        graph_to_canonical_json(kind, result.program())
    }))
}
...
fn command_diff(
    ...
    let difference = semantic_diff(base.result.program(), head.result.program());
...
fn command_fix(cwd: &Path, arguments: &[String]) -> Result<CommandOutput, CliError> {
    ...
    let sources: Vec<(String, Arc<str>)> = if target.is_file() {
        let (logical, source) = selected_source(&loaded, &target)?;
        vec![(logical.to_owned(), Arc::from(source))]
    } else {
        loaded
            .sources
            .iter()
...
impl DependencyFetcher for NativeDependencyResolver {
    fn fetch(
        &mut self,
        dependency: &crate::frontend::Dependency,
...
fn command_completion(arguments: &[String]) -> Result<CommandOutput, CliError> {
    ...
    let script = match arguments[0].as_str() {
        "bash" => {
            "complete -W 'check resolve explain graph diff fix policy sandbox doctor completion version lsp auth' workflow-verifier\n"
        }
        "zsh" => {
            "#compdef workflow-verifier\n_arguments '1:command:(check resolve explain graph diff fix policy sandbox doctor completion version lsp auth)'\n"
        }
        "fish" => {
            "complete -c workflow-verifier -f -a 'check resolve explain graph diff fix policy sandbox doctor completion version lsp auth'\n"
        }
        "powershell" => {
            "Register-ArgumentCompleter -Native -CommandName workflow-verifier -ScriptBlock { param($wordToComplete) 'check','resolve','explain','graph','diff','fix','policy','sandbox','doctor','completion','version','lsp','auth' | Where-Object { $_ -like \"$wordToComplete*\" } }\n"
        }
        _ => {
            return Err(CliError::invalid(
                "completion requires bash, zsh, fish, or powershell",
...
fn analyze_target(
    ...
    analyze_target_with_loader(target, common, context, false)
}

fn analyze_sandbox_target(
    target: &Path,
    common: &CommonOptions,
    context: &AnalysisContext,
) -> Result<AnalyzedWorkspace, CliError> {
    analyze_target_with_loader(target, common, context, true)
}

fn analyze_target_with_loader(
    target: &Path,
    common: &CommonOptions,
    context: &AnalysisContext,
    sandbox_snapshot: bool,
) -> Result<AnalyzedWorkspace, CliError> {
    let (loaded, config_snapshot, config) = if sandbox_snapshot {
        load_sandbox_workspace_and_config(target, common)?
    } else {
        load_workspace_and_config(target, common)?
    };
    let persona = common.persona.unwrap_or(config.persona);
    let result = context
        .analyzer()
        .analyze(AnalysisInput {
            sources: Arc::clone(&loaded.analysis_sources),
            overlays: BTreeMap::new(),
            roots: loaded.roots.clone(),
            config: config_snapshot,
            lock: loaded.lock_snapshot.clone(),
            persona,
...
fn load_workspace_and_config(
    ...
    let explicit_config = common.policy.as_ref().or(common.config.as_ref());
    if let Some(policy) = &common.policy {
        let canonical = policy.canonicalize().map_err(|error| {
            CliError::invalid(format!("cannot open config {}: {error}", policy.display()))
        })?;
    ...
    let mut options = LoadOptions::new(&target);
    options.config = explicit_config.cloned();
    options.config_trust = if common.policy.is_some() || common.trust_repository_config {
        "trusted".to_owned()
    } else {
        "repository".to_owned()
    };
    options.lockfile.clone_from(&common.lockfile);
    options.persona = common.persona.unwrap_or(Persona::Gate);
    options.strict = common.strict;
    let input = WorkspaceLoader::new(CliWorkspaceFileSystem)
        .load(options)
        .map_err(|error| CliError::invalid(error.to_string()))?;
    let config_snapshot = input.config.clone();
    let config = parse_loaded_config(&config_snapshot)?;
    let lock = parse_loaded_lock(&input.lock)?;
    let sources = input
        .sources
        .iter()
    ...
    Ok((
        LoadedWorkspace {
            root,
            sources,
            analysis_sources: Arc::clone(&input.sources),
            authenticated: None,
            roots: input.roots,
            lock,
            lock_snapshot: input.lock,
        },
        config_snapshot,
        config,
    ))
...
fn parse_loaded_lock(snapshot: &LockSnapshot) -> Result<Lockfile, CliError> {
    if snapshot.bytes.is_empty() {
        return Lockfile::new([]).map_err(CliError::internal);
    }
    let source = std::str::from_utf8(&snapshot.bytes)
        .map_err(|error| CliError::invalid(format!("lockfile is not UTF-8: {error}")))?;
    Lockfile::parse(source).map_err(CliError::invalid)
}

fn load_sandbox_workspace_and_config(
    target: &Path,
    common: &CommonOptions,
) -> Result<(LoadedWorkspace, ConfigSnapshot, Config), CliError> {
    let target = target.canonicalize().map_err(|error| {
    ...
    for (logical, contents) in authenticated.regular_files() {
        ...
        sources.insert(logical.to_owned(), Arc::clone(&source));
        analysis_sources.push(
            AnalysisSource::new(logical, source)
                .map_err(|error| CliError::invalid(error.to_string()))?,
        );
    }
    analysis_sources.sort_by(|left, right| left.path.cmp(&right.path));
    let (lock, lock_snapshot) = load_lock(&root, common.lockfile.as_deref(), Some(&authenticated))?;
    Ok((
        LoadedWorkspace {
            root,
            sources,
            analysis_sources: analysis_sources.into(),
...
fn render_text(result: &AnalysisOutcome, sources: &BTreeMap<String, Arc<str>>) -> String {
    let diagnostics = result.diagnostics();
    if diagnostics.is_empty() {
        return format!(
            "No findings. {} workflow source(s) analyzed.\n",
            result.program().sources.len()
        );
    }
    let mut output = String::new();
    for diagnostic in diagnostics {
...
fn check_exit_code(result: &AnalysisOutcome) -> i32 {
    i32::try_from(result.gate().exit_code)
        .unwrap_or_else(|_| process_exit_code(EXIT_CODE_INTERNAL_FAILURE))
}

fn frontend_error(problems: &[crate::frontend::FrontendProblem]) -> CliError {
    CliError::invalid(frontend_message(problems))
}

fn frontend_message(problems: &[crate::frontend::FrontendProblem]) -> String {
    problems
        .iter()
...
mod tests {
    ...
    use crate::application::auth::SecretString;
    use crate::engine::Analyzer;
    use crate::product::ResolverOrigin;
    ...
    #[test]
    fn analysis_context_lazily_initializes_one_stateless_analyzer() {
        let context = AnalysisContext::default();
        ...
        let first = context.analyzer_with(|| {
            initializations.fetch_add(1, Ordering::Relaxed);
            Analyzer::default()
        });
        let second = context.analyzer_with(|| {
            initializations.fetch_add(1, Ordering::Relaxed);
            Analyzer::default()
        });
