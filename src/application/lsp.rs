...
use crate::engine::{
    AnalysisInput, AnalysisOutcome, AnalysisSession, AnalysisSource, CancellationToken,
    ConfigSnapshot, LockSnapshot,
};
use crate::foundation::{
    Budget, JsonValue, Span, Utf16Position, byte_to_utf16, content_digest, utf16_to_byte,
};
use crate::product::{EXIT_CODE_INVALID_INPUT, EXIT_CODE_PASS, FixProposal, GraphKind};
...
struct Server {
    ...
    semantic_cache: RefCell<BTreeMap<String, Vec<u32>>>,
    shutdown: bool,
}
...
impl Server {
    fn new(cancellations: Arc<RequestCancellations>, observed: Arc<ObservedDocuments>) -> Self {
        Self {
            ...
            cancellations,
            observed,
            session: RefCell::new(AnalysisSession::default()),
            semantic_cache: RefCell::new(BTreeMap::new()),
            shutdown: false,
        }
    }
    ...
    fn handle(&mut self, message: &Value) -> Vec<Value> {
        ...
        match method {
            ...
            "workspace/didChangeWatchedFiles" | "workspace/didChangeConfiguration" => {
                *self.session.borrow_mut() = AnalysisSession::default();
                Vec::new()
            }
    ...
    fn diagnostics(&self, document: &Document, cancellation: &CancellationToken) -> Vec<Value> {
        let Ok(syntax) = self.session.borrow_mut().parse_document(
        ...
        self.analysis(document, cancellation)
            .map(|result| {
                result
                    ...
                    .iter()
                    .filter(|diagnostic| {
                        result.program().source_path_for(diagnostic.span.source)
    ...
    fn analysis_with_replacements(
        ...
        let result = (|| {
                ...
                .filter(|candidate| candidate.workspace == document.workspace)
            {
                ...
                if include {
                    sources.push(
                        AnalysisSource::new(candidate.logical_path.clone(), Arc::<str>::from(text))
                            .map_err(|error| error.to_string())?,
                    );
                }
            }
            let mut input = AnalysisInput::new(sources).map_err(|error| error.to_string())?;
            input.overlays = BTreeMap::new();
            input.roots = None;
            input.config = ConfigSnapshot::default();
            input.lock = LockSnapshot::default();
            input.persona = Persona::Audit;
            input.budget = Budget::default();
    ...
    fn hover(&self, params: &Value, cancellation: &CancellationToken) -> Result<Value, String> {
        ...
        if let Some(diagnostic) = self
            ...
            .ok()
            .into_iter()
            .flat_map(|result| result.diagnostics().to_vec())
            .find(|diagnostic| diagnostic.span.contains(byte))
        {
    ...
    fn document_symbols(
        ...
        Ok(Value::Array(
            result
                ...
                .iter()
                .filter(|symbol| {
                    result.program().source_path_for(symbol.source)
                        == Some(document.logical_path.as_str())
                })
                .map(|symbol| {
                    json!({
    ...
    fn semantic_token_delta(&self, params: &Value) -> Result<Value, String> {
        ...
        if params.get("previousResultId").and_then(Value::as_str) == Some(&current) {
            ...
            let previous = params
                ...
                .and_then(Value::as_str)
                .and_then(|id| self.semantic_cache.borrow().get(id).cloned());
            self.semantic_cache
                .borrow_mut()
    ...
    fn code_actions(
        ...
        let unused = before
            ...
            .iter()
            .filter(|diagnostic| {
                before.program().source_path_for(diagnostic.span.source)
                    == Some(document.logical_path.as_str())
                    && diagnostic.rule_id == "WV-PERM-001"
            })
...
fn severe_diagnostics(result: &AnalysisOutcome) -> BTreeSet<(String, String)> {
    result
        .diagnostics()
        .iter()
        .filter(|diagnostic| matches!(diagnostic.severity, Severity::Critical | Severity::Error))
        .map(|diagnostic| (diagnostic.rule_id.clone(), diagnostic.message.clone()))
        .collect()
}
...
fn analysis_projection(result: &AnalysisOutcome, rename: Option<(&str, &str)>) -> Vec<String> {
    let source = result.graph(GraphKind::All).to_canonical_json();
    let Ok(JsonValue::Object(mut graph)) = JsonValue::parse(&source) else {
        return Vec::new();
    };
    graph.remove("digest");
    graph.remove("schema");
    let mut node_values = BTreeMap::new();
    if let Some(JsonValue::Array(nodes)) = graph.get_mut("nodes") {
        for node in nodes.iter_mut() {
            let id = node.member("id").and_then(JsonValue::as_i64).unwrap_or(-1);
            ...
            remove_analysis_spans(node);
            node_values.insert(id, node.clone());
        }
        nodes.sort_by_cached_key(JsonValue::canonical);
    }
    if let Some(JsonValue::Array(edges)) = graph.get_mut("edges") {
        for edge in edges.iter_mut() {
            if let JsonValue::Object(fields) = edge {
                if let Some(condition) = fields.get_mut("condition") {
                    normalize_identifier_value(condition, rename);
                }
    ...
    if let Some(JsonValue::Array(entrypoints)) = graph.get_mut("entrypoints") {
        for entrypoint in entrypoints.iter_mut() {
            let id = entrypoint.as_i64().unwrap_or(-1);
            *entrypoint = node_values
                .get(&id)
                .cloned()
                .unwrap_or_else(|| JsonValue::String(format!("missing:{id}")));
        }
    ...
    vec![JsonValue::Object(graph).canonical()]
}
...
mod diagnostics_cache_tests {
    ...
    #[test]
    fn diagnostics_reuses_the_session_parse_for_analysis() {
        ...
        let statistics = server.session.borrow().statistics();
...
mod performance_tests {
    ...
    fn transitive_invalidation_p95_is_at_most_500_milliseconds() {
        ...
        let mut previous_digest = initial.check_report().analysis_digest();
        let mut samples = Vec::new();
        for marker in 1..=PERFORMANCE_SAMPLE_COUNT {
            let action = server
                .documents
            ...
            samples.push(started.elapsed());
            let digest = result.check_report().analysis_digest();
