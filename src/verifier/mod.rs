...
use crate::domain::{
    AbstractValue, Capability, Condition, Edge, EdgeKind, Graph, Node, NodeId, NodeKind,
    ObservableEffect, Program, Provider, Secrecy, Source, Trust, UnknownReason, Value,
};
use crate::foundation::{
    DependencyClass, Digest, DigestBuilder, JsonValue, SourceId, Span, classify_reference,
    normalize_slashes, valid_content_digest,
};
...
/// Evidence-bearing fields grouped separately from the diagnostic headline.
pub struct DiagnosticDetails<C, E> {
    trace: Vec<TraceHop>,
    capabilities: C,
    evidence: E,
    fix: Option<Fix>,
...
impl Diagnostic {
        ...
        E: IntoIterator<Item = String>,
    {
        ...
        let evidence: Vec<_> = evidence
            ...
            .into_iter()
            .collect();
        let mut identity = Digest::builder(b"workflow-verifier/diagnostic/1");
        identity
            .add(&rule_id)
            .add(severity.name())
            .add(confidence.name())
            .add(&message);
        add_span_identity(&mut identity, span);
        identity
            .add(b"trace")
            .add(u64::try_from(trace.len()).unwrap_or(u64::MAX).to_be_bytes());
        for hop in &trace {
            identity.add(&hop.label);
            add_span_identity(&mut identity, hop.span);
        }
        identity.add(b"capabilities").add(
            u64::try_from(capabilities.len())
                .unwrap_or(u64::MAX)
                .to_be_bytes(),
        );
        for capability in &capabilities {
            identity.add(capability.name());
        }
        identity.add(b"evidence").add(
        ...
        if let Some(fix) = &fix {
            ...
            match &fix.replacement {
                Some(replacement) => {
                    identity.add(b"replacement:present").add(replacement);
                }
                None => {
                    identity.add(b"replacement:absent");
                }
            }
            if let Some(fix_span) = fix.span {
                identity.add(b"span:present");
                add_span_identity(&mut identity, fix_span);
            } else {
                identity.add(b"span:absent");
            }
        } else {
            identity.add(b"fix:absent");
        }
        let id = format!("diag_{}", identity.finish().hex());
        Self {
            id,
            rule_id,
            severity,
        ...
        let trace = self
            ...
            .map(|hop| {
                JsonValue::Object(BTreeMap::from([
                    ...
                    (
                        "node_id".to_owned(),
                        JsonValue::Integer(i64::from(hop.node_id.0)),
                    ),
...
fn add_span_identity(identity: &mut DigestBuilder, span: Span) {
    identity
        .add(span.source.0.to_be_bytes())
        .add(span.start.byte.to_be_bytes())
        .add(span.start.line.to_be_bytes())
        .add(span.start.column.to_be_bytes())
        .add(span.stop.byte.to_be_bytes())
        .add(span.stop.line.to_be_bytes())
        .add(span.stop.column.to_be_bytes());
}
...
struct GraphIndex<'a> {
    ...
    outgoing: BTreeMap<NodeId, Vec<(NodeId, EdgeKind)>>,
...
impl<'a> GraphIndex<'a> {
    fn new(graph: &'a Graph) -> Self {
        ...
        let mut outgoing: BTreeMap<NodeId, Vec<(NodeId, EdgeKind)>> = BTreeMap::new();
        let mut incoming: BTreeMap<NodeId, Vec<(NodeId, EdgeKind)>> = BTreeMap::new();
    ...
    fn shortest_path(&self, from: NodeId, to: NodeId, kinds: &[EdgeKind]) -> Option<Vec<&'a Node>> {
        let mut queue = VecDeque::from([from]);
        let mut previous: BTreeMap<NodeId, Option<NodeId>> = BTreeMap::from([(from, None)]);
    ...
    fn reachable(&self, from: NodeId) -> Vec<&'a Node> {
        let mut queue = VecDeque::from([from]);
    ...
    fn cycles(&self, kinds: &[EdgeKind]) -> Vec<Vec<NodeId>> {
        fn visit(
            index: &GraphIndex<'_>,
            kinds: &[EdgeKind],
            current: NodeId,
    ...
    fn dominates(&self, dominator: NodeId, node: NodeId) -> bool {
        ...
        let entrypoints: BTreeSet<NodeId> = if self.graph.entrypoints.is_empty() {
            self.nodes
                .keys()
            ...
            self.graph.entrypoints.iter().copied().collect()
        };
    ...
    fn path_avoiding(&self, from: NodeId, to: NodeId, avoided: NodeId) -> bool {
        if from == avoided {
            return false;
        }
        let mut queue = VecDeque::from([from]);
...
fn canonical_cycle(mut cycle: Vec<NodeId>) -> Vec<NodeId> {
    ...
    for offset in 0..cycle.len() {
        ...
        value.push(value.first().copied().unwrap_or_default());
...
impl Dataflow {
    fn solve(graph: &Graph) -> Self {
        ...
        let mut propagating_edges: Vec<_> = graph
            ...
            .collect();
        propagating_edges.sort_by_key(|edge| (edge.from, edge.to, edge.kind));
...
fn data_trace(
    ...
    for source in candidates
        .iter()
        .copied()
        .filter(|node| node.attributes.values().any(AbstractValue::is_untrusted))
        ...
        .filter(|node| node.kind == NodeKind::Command)
    {
        ...
        if value.is_untrusted() && unsafe_untrusted_flow(graph, index, dataflow, command, &summary)
        {
            ...
            diagnostics.push(Diagnostic::new(
                ...
                command.span,
                Diagnostic::details(data_trace(graph, index, dataflow, command),
                [Capability::Shell],
                ...
                diagnostics.push(Diagnostic::new(
                    ...
                    Diagnostic::details(
                        ...
                        Some(Fix {
                            ...
                            description:
                                "resolve and replace the reference with an immutable revision"
                                    .to_owned(),
                            replacement: None,
                            span: Some(call.span),
                        }),
...
fn correctness_rule(graph: &Graph, index: &GraphIndex<'_>) -> RuleResult {
    ...
    for issue in graph.validate() {
        let span = issue
            .node_ids
            .iter()
            .find_map(|id| index.nodes.get(id).map(|node| node.span))
            .unwrap_or_default();
        diagnostics.push(Diagnostic::new(
            "WV-CORRECT-001",
            Severity::Error,
            ...
            issue.message,
            span,
            Diagnostic::details(Vec::new(), [], [issue.code], None),
        ));
    }
...
pub fn compose_program(graphs: &[Graph]) -> Graph {
    compose_program_owned(graphs.to_vec())
}

/// Consuming composition path used by the analyzer so lowered nodes and edges
/// move into their final dense storage without cloning intermediate graphs.
#[must_use]
pub fn compose_program_owned(graphs: Vec<Graph>) -> Program {
    let (sources, source_ids) = collect_program_sources(&graphs);
    let (node_maps, node_capacity, edge_capacity) = build_node_maps(&graphs);
    let (resolved_calls, local_edges) = resolve_local_call_edges(&graphs, &node_maps);
    let local_edge_count = local_edges.len();
    let (nodes, mut edges, entrypoints) = merge_program_fragments(
        graphs,
        &node_maps,
        &source_ids,
        &resolved_calls,
        node_capacity,
        edge_capacity.saturating_add(local_edge_count),
    );
    edges.extend(local_edges);
    append_cross_source_resource_edges(&nodes, &mut edges);
...
type SourceKey = (String, Provider);
type NodeMap = HashMap<NodeId, NodeId>;

fn collect_program_sources(graphs: &[Graph]) -> (Vec<Source>, BTreeMap<SourceKey, SourceId>) {
    ...
    for graph in graphs {
        source_keys.extend(
            graph
                .sources
                .iter()
                .map(|source| (normalize_slashes(&source.path), source.provider)),
        );
    }
    let sources: Vec<_> = source_keys
        .into_iter()
        .enumerate()
        .map(|(index, (path, provider))| Source {
            id: SourceId(u32::try_from(index).unwrap_or(u32::MAX)),
            path,
            provider,
        })
        .collect();
    let source_ids: BTreeMap<_, _> = sources
        .iter()
        .map(|source| ((source.path.clone(), source.provider), source.id))
        .collect();
    (sources, source_ids)
}

fn build_node_maps(graphs: &[Graph]) -> (Vec<NodeMap>, usize, usize) {
    let node_capacity: usize = graphs.iter().map(|graph| graph.nodes.len()).sum();
    let edge_capacity: usize = graphs.iter().map(|graph| graph.edges.len()).sum();
    let mut node_maps = Vec::with_capacity(graphs.len());
    let mut next_node = 0usize;
    for graph in graphs {
        let mut node_map = HashMap::with_capacity(graph.nodes.len());
        for node in &graph.nodes {
            let next = NodeId(u32::try_from(next_node).unwrap_or(u32::MAX));
            next_node = next_node.saturating_add(1);
            node_map.insert(node.id, next);
        }
        node_maps.push(node_map);
    }
    (node_maps, node_capacity, edge_capacity)
}

fn resolve_local_call_edges(
    graphs: &[Graph],
    node_maps: &[NodeMap],
) -> (BTreeSet<NodeId>, Vec<Edge>) {
    let mut resolved_calls = BTreeSet::new();
    let mut local_edges = Vec::new();
    for (caller_index, caller) in graphs.iter().enumerate() {
        for call in caller
            .nodes
            .iter()
            .filter(|node| node.kind == NodeKind::Call)
        {
            ...
            let Some(composed_call_id) = node_maps[caller_index].get(&call.id).copied() else {
                continue;
            };
            if caller.source_path() == target.source_path() {
                continue;
            }
            resolved_calls.insert(composed_call_id);
            let Some(target_index) = graphs.iter().position(|graph| std::ptr::eq(graph, target))
            else {
                continue;
            };
            for entrypoint in &target.entrypoints {
                let Some(entrypoint) = node_maps[target_index].get(entrypoint).copied() else {
                    continue;
                };
    ...
    (resolved_calls, local_edges)
}

fn merge_program_fragments(
    graphs: Vec<Graph>,
    node_maps: &[NodeMap],
    source_ids: &BTreeMap<SourceKey, SourceId>,
    resolved_calls: &BTreeSet<NodeId>,
    node_capacity: usize,
    edge_capacity: usize,
) -> (Vec<Node>, Vec<Edge>, Vec<NodeId>) {
    let mut nodes = Vec::with_capacity(node_capacity);
    let mut edges = Vec::with_capacity(edge_capacity);
    let mut entrypoints = Vec::new();
    for (graph, node_map) in graphs.into_iter().zip(node_maps) {
        let Program {
            sources: fragment_sources,
            nodes: fragment_nodes,
            edges: fragment_edges,
            entrypoints: fragment_entrypoints,
        } = graph;
        let source_remap: HashMap<_, _> = fragment_sources
            .iter()
            .filter_map(|source| {
                source_ids
                    .get(&(normalize_slashes(&source.path), source.provider))
                    .copied()
                    .map(|id| (source.id, id))
            })
            .collect();
        for mut node in fragment_nodes {
            node.remap_sources(&source_remap);
            let old = node.id;
            node.id = node_map.get(&old).copied().unwrap_or_default();
            if resolved_calls.contains(&node.id) {
                node.unknown = None;
            }
            nodes.push(node);
        }
        for mut edge in fragment_edges {
...
fn append_cross_source_resource_edges(nodes: &[Node], edges: &mut Vec<Edge>) {
    ...
    for writer in &writers {
        for reader in readers.iter().filter(|reader| {
            reader.name == writer.name
                && reader.id != writer.id
                && reader.span.source != writer.span.source
        }) {
            edges.push(Edge::new(
                EdgeKind::Persist,
                writer.id,
...
fn local_call_target<'a>(graphs: &'a [Graph], caller: &Graph, call: &Node) -> Option<&'a Graph> {
    ...
    let caller_source = normalize_slashes(caller.source_path());
...
fn normalized_source(graph: &Graph) -> String {
    normalize_slashes(graph.source_path())
        .trim_start_matches("./")
        .to_owned()
}
...
#[must_use]
pub fn verify_program(persona: Persona, program: &Program) -> VerificationResult {
    verify(persona, program)
}
...
mod tests {
    ...
    fn test_node_in(_source: &str, kind: NodeKind, name: &str) -> Node {
    ...
    fn public_names_json_and_total_diagnostic_order_are_exact() {
        ...
        let first = Diagnostic::new(
            ...
            "first",
            test_span(),
            Diagnostic::details(Vec::new(), [], Vec::<String>::new(), None),
        );
    ...
    fn graph_index_obeys_edge_filters_reachability_dominance_and_cycle_order() {
        ...
        graph.add_entrypoint(entry.id);
        graph.add_edge(Edge::simple(EdgeKind::Control, entry.id, gate.id));
        graph.add_edge(Edge::simple(EdgeKind::Control, gate.id, sink.id));
        graph.add_edge(Edge::simple(EdgeKind::Data, entry.id, bypass.id));
        graph.finalize();
        let id = |name: &str| {
            graph
                .nodes
        ...
        let gate_id = id("gate");
        let sink_id = id("sink");
        let bypass_id = id("bypass");

        let index = GraphIndex::new(&graph);
        assert_eq!(
            index
                .shortest_path(entry_id, sink_id, &[EdgeKind::Control])
                .expect("control path")
                .iter()
        ...
        assert!(
            index
                .shortest_path(entry_id, bypass_id, &[EdgeKind::Control])
                .is_none()
        );
        assert_eq!(
            index
        ...
        assert!(index.dominates(entry_id, entry_id));
        assert!(index.dominates(gate_id, sink_id));
        assert!(!index.path_avoiding(entry_id, sink_id, gate_id));

        let mut bypassed = graph.clone();
        bypassed.add_edge(Edge::simple(EdgeKind::Control, entry_id, bypass_id));
        bypassed.add_edge(Edge::simple(EdgeKind::Call, bypass_id, sink_id));
        bypassed.finalize();
        let bypassed_index = GraphIndex::new(&bypassed);
        assert!(bypassed_index.path_avoiding(entry_id, sink_id, gate_id));
        assert!(!bypassed_index.dominates(gate_id, sink_id));
        ...
        assert!(inferred_index.dominates(gate_id, sink_id));

        assert_eq!(
            canonical_cycle(vec![NodeId(2), NodeId(3), NodeId(1), NodeId(2),]),
            vec![NodeId(1), NodeId(2), NodeId(3), NodeId(1),]
        );
        assert_eq!(canonical_cycle(Vec::new()), Vec::<NodeId>::new());
        assert_eq!(canonical_cycle(vec![NodeId(7)]), vec![NodeId(7), NodeId(7)]);
    }
    ...
    fn dataflow_and_unknown_reason_collection_follow_semantic_edges_only() {
        ...
        let composite = AbstractValue {
            value_type: crate::domain::ValueType::Dynamic,
        ...
        graph.add_edge(Edge::simple(EdgeKind::Data, source.id, middle.id));
        graph.add_edge(Edge::simple(EdgeKind::Read, middle.id, target.id));
        graph.add_edge(Edge::simple(EdgeKind::Control, source.id, control_only.id));
        graph.finalize();
        ...
        let node = |name: &str| {
            graph
                ...
                .iter()
                .find(|node| node.name == name)
                .expect("dataflow node")
        };
        let source = node("source");
        let middle = node("middle");
        let target = node("target");
        let control_only = node("control-only");
    ...
    fn dataflow_handles_high_fan_in_cycles_and_self_edges_canonically() {
        ...
        for index in 0..SOURCE_COUNT {
            ...
            graph.add_edge(Edge::simple(EdgeKind::Data, source.id, hub.id));
            graph.add_node(source);
        }
        graph.add_edge(Edge::simple(EdgeKind::Read, hub.id, cycle_a.id));
        graph.add_edge(Edge::simple(EdgeKind::Write, cycle_a.id, cycle_b.id));
        graph.add_edge(Edge::simple(EdgeKind::Persist, cycle_b.id, hub.id));
        graph.add_edge(Edge::simple(EdgeKind::Data, hub.id, hub.id));
        graph.finalize();
        ...
        let expected_origins: Vec<_> = (0..SOURCE_COUNT)
            ...
            .collect();
        for node in [node("fan-in-hub"), node("cycle-a"), node("cycle-b")] {
            let value = dataflow.at(node);
            ...
            assert_eq!(
                value.value,
                Value::String(crate::domain::abstract_value::StringValue::Top)
            );
        }
        ...
        for name in ["fan-in-hub", "cycle-a", "cycle-b"] {
            let original = graph
                .nodes
                .iter()
                .find(|node| node.name == name)
            ...
            let reordered_node = reordered
                ...
                .find(|node| node.name == name)
                .expect("reordered");
            assert_eq!(dataflow.at(original), reordered_dataflow.at(reordered_node));
        }
    }
    ...
    fn authorization_gate_trust_and_environment_uncertainty_are_explicit() {
        ...
        graph.add_edge(Edge::simple(EdgeKind::Grant, environment.id, sink.id));
        graph.finalize();
        let sink = graph
            .nodes
            .iter()
            .find(|node| node.name == sink.name)
            .expect("sink survives finalization");
        assert_eq!(
    ...
    fn program_composition_links_local_entrypoints_and_avoids_same_file_recursion() {
        ...
        target.add_entrypoint(entrypoint.id);
        let program = compose_program(&[caller, target]);
        let call_id = program
            .nodes
            .iter()
            .find(|node| node.name == call.name)
            .expect("composed call")
            .id;
        let entrypoint_id = program
            .nodes
            .iter()
            .find(|node| node.name == entrypoint.name)
            .expect("composed entrypoint")
            .id;
        assert_eq!(
            program
                .find_node(call_id)
        ...
        for kind in [EdgeKind::Call, EdgeKind::Control] {
            assert!(program.edges.iter().any(|edge| {
                edge.kind == kind && edge.from == call_id && edge.to == entrypoint_id
            }));
        }

        let mut same_file = Graph::empty(Provider::Github, "action.yml");
        let recursive_call = Node::new(
            Provider::Github,
            NodeKind::Call,
            "self",
            crate::domain::Phase::Run,
        ...
        same_file.add_entrypoint(same_entrypoint.id);
        let same_program = compose_program(&[same_file]);
        let recursive_call_id = same_program
            .nodes
            .iter()
            .find(|node| node.name == recursive_call.name)
            .expect("recursive call")
            .id;
        let same_entrypoint_id = same_program
            .nodes
            .iter()
            .find(|node| node.name == same_entrypoint.name)
            .expect("same-file entrypoint")
            .id;
        assert_eq!(
            same_program
                .find_node(recursive_call_id)
                .and_then(|node| node.unknown.as_ref()),
            Some(&UnknownReason::RecursiveCall("action.yml".to_owned()))
        );
        assert!(
            !same_program
                .edges
                .iter()
                .any(|edge| edge.from == recursive_call_id && edge.to == same_entrypoint_id)
        );
    }
    ...
    fn program_composition_does_not_invent_cross_file_resource_edges() {
        ...
        for target in [
            ...
            producer.add_edge(Edge::simple(EdgeKind::Write, producer_step.id, target.id));
        }
        ...
        for source in [
            ...
            consumer.add_edge(Edge::simple(EdgeKind::Read, source.id, consumer_step.id));
        }

        let program = compose_program(&[producer, consumer]);
        let cross_file_edges: Vec<_> = program
            .edges
            .iter()
            .filter(|edge| edge.label.as_deref() == Some("cross-file resource"))
            .collect();
        assert_eq!(cross_file_edges.len(), 1);
        let edge = cross_file_edges[0];
        assert_eq!(
            program.find_node(edge.from).map(|node| node.name.as_str()),
            Some("legitimate")
        );
        assert_eq!(
            program.source_path_for(program.find_node(edge.from).expect("writer").span.source),
            Some("producer.yml")
        );
        assert_eq!(
            program.find_node(edge.to).map(|node| node.name.as_str()),
            Some("legitimate")
        );
        assert_eq!(
            program.source_path_for(program.find_node(edge.to).expect("reader").span.source),
            Some("consumer.yml")
        );
    }
    ...
    fn supply_and_permission_rules_cover_locked_mutable_used_and_unknown_cases() {
        ...
        locked.attributes.insert(
            ...
            test_value(
                &Digest::of_bytes(b"locked dependency").to_string(),
                Trust::Trusted,
                Secrecy::Public,
            ),
        );
        ...
        empty_digest_set.attributes.insert(
            "dependency.digest".to_owned(),
            AbstractValue {
                value_type: crate::domain::ValueType::String,
                value: Value::String(crate::domain::abstract_value::StringValue::Constants(
                    Vec::new(),
                )),
                trust: Trust::Trusted,
        ...
        used.add_entrypoint(owner.id);
        used.add_edge(Edge::simple(EdgeKind::Control, owner.id, network.id));
        used.finalize();
        assert_eq!(
    ...
    #[expect(
        clippy::too_many_lines,
        reason = "one exhaustive table covers every secret sink and uncertain non-sink outcome"
    )]
    fn secret_rule_distinguishes_each_sink_shape_and_uncertain_non_sink() {
        ...
        uncertain_output.attributes.insert(
            ...
            AbstractValue {
                value_type: crate::domain::ValueType::String,
                value: Value::String(crate::domain::abstract_value::StringValue::Constants(vec![
                    "token".to_owned(),
                ])),
                trust: Trust::Trusted,
                secrecy: Secrecy::Unknown(vec![secrecy_reason.clone()]),
