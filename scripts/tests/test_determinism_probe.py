...
class DeterminismProbeTests(unittest.TestCase):
    def test_ci_and_local_tasks_probe_the_rust_cli_on_all_five_platforms(self) -> None:
        ...
        self.assertIn("cargo build --locked -p workflow-verifier", determinism_job)
        self.assertIn("_determinism/determinism-linux-arm64", ci)
        self.assertIn("cargo build --locked -p workflow-verifier", justfile)
    ...
    def test_commands_use_the_same_relative_fixture_and_never_enable_network(self) -> None:
        ...
        self.assertNotIn("--cache-mode", commands[0])
    ...
    def test_manifest_is_order_independent_and_hashes_exact_bytes(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            first = artifact_manifest(root, ["fix.diff", "report.json", "workflow-verifier.lock"])
            second = artifact_manifest(root, ["workflow-verifier.lock", "fix.diff", "report.json"])
            ...
            self.assertEqual(
                ...
                ["fix.diff", "report.json", "workflow-verifier.lock"],
            )
