...
class BenchmarkFixtureTests(unittest.TestCase):
    ...
    def test_arcade_scale_fixture_is_large_deterministic_and_single_delta(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(before.count(b"name: arcadeVariable"), 778)
            self.assertEqual(before.count(b"environment: production-"), 2)
            self.assertEqual(before.count(b"- stage:"), 3)
            self.assertNotIn(b"script:", before)
