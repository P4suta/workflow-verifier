...
| Mutation evidence transport | Concurrent Dune stages report lock failures as killed mutants, and invalid mutant output corrupts JSON | Every managed stage has a private build root; strict JSON retains raw SHA-256 and encoding-error evidence | Scheduling and byte sanitization live at the runner boundary, never in analyzer tests |
