...
| Mutation evidence transport | Concurrent Dune stages report lock failures as killed mutants, invalid mutant output corrupts JSON, or unbounded hosted fan-out loses every shard to runner reclamation | Every managed stage has a private build root; strict JSON retains raw SHA-256 and encoding-error evidence; the hosted matrix applies explicit four-worker backpressure | Scheduling and byte sanitization live at the runner boundary, never in analyzer tests |
