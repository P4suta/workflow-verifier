...
use workflow_verifier_runner_protocol::{Evidence, PlanStatus, RunResult, validate_plan};
...
"#;
    let compilation = workflow_verifier_frontend::compile_auto(
        ".github/workflows/ci.yml",
        source,
        workflow_verifier_foundation::Budget::default(),
    )
    .expect("workflow");
...
fn scenario_planner_follows_local_composite_action_call_edges() {
    let workflow = workflow_verifier_frontend::compile_auto(
        ".github/workflows/ci.yml",
        "on: workflow_dispatch\njobs:\n  build:\n    steps:\n      - uses: ./.github/actions/demo\n",
        workflow_verifier_foundation::Budget::default(),
    )
    .expect("workflow");
    let action = workflow_verifier_frontend::compile_auto(
